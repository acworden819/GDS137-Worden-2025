...
function animate()
{
	...
	*/
	...
	while(platform0.hitTestPoint({x:player.bottomLeft().x, y:player.bottomLeft().y}) && player.vy >=0)
	{
		player.y--;
		player.vy = 0;
		player.canJump = true;
	...
	while(platform1.hitTestPoint({x:player.bottomRight().x, y:player.bottomRight().y}) && player.vy >=0)
	{
		player.y--;
		player.vy = 0;
		player.canJump = true;
